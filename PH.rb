...
module PH
  ...
  '''
  module SKP
    def self.drawFace(argFaceCoords)
      faceCoords = nil

      if argFaceCoords.class == Hash
        raise "argFaceCoords is not type of Hash" unless argFaceCoords.class == Hash
        raise "argFaceCoords Hash as not X as Key" unless argFaceCoords.keys.include? "X"
        raise "argFaceCoords Hash as not an Array as X Key value" unless argFaceCoords["X"].is_a? Array
        raise "argFaceCoords Hash as not an Array[2] as X Key value" unless argFaceCoords["X"].length == 2
        raise "argFaceCoords Hash as not an Array as Y Key value" unless argFaceCoords["Y"].is_a? Array
        raise "argFaceCoords Hash as not an Array[2] as Y Key value" unless argFaceCoords["Y"].length == 2
        raise "argFaceCoords Hash as not an Array as Z Key value" unless argFaceCoords["Z"].is_a? Array
        raise "argFaceCoords Hash as not an Array[2] as Z Key value" unless argFaceCoords["Z"].length == 2

        #Dimension order
        order = argFaceCoords.keys
        ...
        faceCoords = []
        faceCoords << [(argFaceCoords["X"][valuesAtTiming[0]["X"]]).mm, (argFaceCoords["Y"][valuesAtTiming[0]["Y"]]).mm, (argFaceCoords["Z"][valuesAtTiming[0]["Z"]]).mm]
        faceCoords << [(argFaceCoords["X"][valuesAtTiming[1]["X"]]).mm, (argFaceCoords["Y"][valuesAtTiming[1]["Y"]]).mm, (argFaceCoords["Z"][valuesAtTiming[1]["Z"]]).mm]
        faceCoords << [(argFaceCoords["X"][valuesAtTiming[2]["X"]]).mm, (argFaceCoords["Y"][valuesAtTiming[2]["Y"]]).mm, (argFaceCoords["Z"][valuesAtTiming[2]["Z"]]).mm]
        faceCoords << [(argFaceCoords["X"][valuesAtTiming[3]["X"]]).mm, (argFaceCoords["Y"][valuesAtTiming[3]["Y"]]).mm, (argFaceCoords["Z"][valuesAtTiming[3]["Z"]]).mm]


      elsif argFaceCoords.class == Array
        raise "argFaceCoords is not type of Array" unless argFaceCoords.class == Array
        argFaceCoords.each_with_index{|current, index| raise "element at index #{index} of argFaceCoords is not type of Array[3]" unless current.class == Array and current.length == 3}

        faceCoords = argFaceCoords.collect{|coords| coords.collect{|val| val.mm}}
      end
    ...
    def self.drawOBJ(argOBJvalues, argExtrudeVal, argCIname:"", argCDname:nil , argCIpos:[0,0,0], argContainer:Sketchup.active_model)
      raise "argOBJvalues is not type of Array or Hash" unless [Array, Hash].include? argOBJvalues.class
      ...
      if componentDefinition.nil?
        #Define face coordinates
        face_coords = SKP.drawFace(argOBJvalues)
